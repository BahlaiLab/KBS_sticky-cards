...
#richness
#AIC 1252
richness.model<-glm(richness ~ CARD + week + TREAT + offset(log(TRAPS)), data=insects21, family = poisson)
summary(richness.model)
Anova (richness.model)
anova (richness.model)
#card: not sig, week and treat: sig
#results: no sig diff btw cards (p=0.93)
AIC(richness.model)
...
#abundance
##AIC 17070
abundance.model<-glm(abundance ~ CARD + week + TREAT + offset(log(TRAPS)), data=insects21, family = poisson)
...
anova (diversity.model)
#card: not sig, week and treat: sig
#results: no sig diff btw cards (p = 0.19)
AIC(diversity.model)
...
#Ichneumonoidea
Ichneumonoidea<-insects21[c(1:4,26,30)]
...
#Chalcidoidea
Chalcidoidea<-insects21[c(1:4,27,30)]
...
HCONV.plot<-ggplot(HCONV, aes(x = CARD, y = HCONV, fill=CARD))+
  geom_boxplot(outlier.shape = NA)+
  geom_jitter(data = HCONV[which(HCONV$HCONV>0),], shape = 1, size = 0.75, width = 0.2, height = 0.2)+
  theme_grey()+
  theme(legend.position="bottom")+
...
CANTHARID.plot<-ggplot(CANTHARID, aes(x = CARD, y = CANTHARID, fill=CARD))+
  geom_boxplot(outlier.shape = NA)+
  geom_jitter(data = CANTHARID[which(CANTHARID$CANTHARID>0),], shape = 1, size = 0.75, width = 0.2, height = 0.2)+
  theme_bw()+
  theme(legend.position="bottom")+
...
Syrphidae.plot<-ggplot(Syrphidae, aes(x = CARD, y = Syrphidae, fill=CARD))+
  geom_boxplot(outlier.shape = NA)+
  geom_jitter(data = Syrphidae[which(Syrphidae$Syrphidae>0),], shape = 1, size = 0.75, width = 0.2, height = 0.2)+
  theme_bw()+
  theme(legend.position="bottom")+
...
Ichneumonoidea.plot<-ggplot(Ichneumonoidea, aes(x = CARD, y = Ichneumonoidea, fill=CARD))+
  geom_boxplot(outlier.shape = NA)+
  geom_jitter(data = Ichneumonoidea[which(Ichneumonoidea$Ichneumonoidea>0),], shape = 1, size = 0.75, width = 0.2, height = 0.2)+
  theme_bw()+
  theme(legend.position="bottom")+
...
Chalcidoidea.plot<-ggplot(Chalcidoidea, aes(x = CARD, y = Chalcidoidea, fill=CARD))+
  geom_boxplot(outlier.shape = NA)+
  geom_jitter(data = Chalcidoidea[which(Chalcidoidea$Chalcidoidea>0),], shape = 1, size = 0.75, width = 0.2, height = 0.2)+
  theme_bw()+
  theme(legend.position="bottom")+
...
Lepidoptera.plot<-ggplot(Lepidoptera, aes(x = CARD, y = Lepidoptera, fill=CARD))+
  geom_boxplot(outlier.shape = NA)+
  geom_jitter(data = Lepidoptera[which(Lepidoptera$Lepidoptera>0),], shape = 1, size = 0.75, width = 0.2, height = 0.2)+
  theme_grey()+
  theme(legend.position="bottom")+
...
Orthoptera.plot<-ggplot(Orthoptera, aes(x = CARD, y = Orthoptera, fill=CARD))+
  geom_boxplot(outlier.shape = NA)+
  geom_jitter(data = Orthoptera[which(Orthoptera$Orthoptera>0),], shape = 1, size = 0.75, width = 0.2, height = 0.2)+
  theme_bw()+
  theme(legend.position="bottom")+
...
#richness
#AIC 1917
richness.model<-glm(richness ~ CARDYEAR + week + TREAT + offset(log(TRAPS)), data=old20_old21, family=poisson)
summary(richness.model)
...
#Plots of individual taxa for old cards
...
HAXY.plot_old<-ggplot(HAXY_old, aes(x = CARDYEAR, y = HAXY, fill=CARDYEAR))+
  geom_boxplot(outlier.shape = NA)+
  geom_jitter(data = HAXY_old[which(HAXY_old$HAXY>0),], shape = 1, size = 0.75, width = 0.2, height = 0.2)+
  theme_bw()+
  theme(legend.position="bottom")+
  labs(title="", x="", y="*Harmonia axyridis*")+
  theme(axis.title.y = ggtext::element_markdown())+ 
...
CANTHARID.plot_old<-ggplot(CANTHARID_old, aes(x = CARDYEAR, y = CANTHARID, fill=CARDYEAR))+
  geom_boxplot(outlier.shape = NA)+
  geom_jitter(data = CANTHARID_old[which(CANTHARID_old$CANTHARID>0),], shape = 1, size = 0.75, width = 0.2, height = 0.2)+
  theme_bw()+
  theme(legend.position="bottom")+
  labs(title="", x="", y="Cantharidae")+
  theme(axis.title.y = ggtext::element_markdown())+ 
...
pdf("taxa.plots_old.pdf", height=10, width=10) #height and width in inches
taxa.plots_old
dev.off()
...
#CARDYEAR diversity by card type
...
#indivdual taxa for poster
ind_boxplots <- ggarrange(HAXY.plot_old, HAXY.plot, HAXY.plot_new,
                          CANTHARID.plot_old, CANTHARID.plot, CANTHARID.plot_new,
                          ncol = 3, nrow = 2,
                          labels = c("A", "B", "C", "D", "E", "F"),
                          common.legend = TRUE, legend = "bottom")
ind_boxplots

pdf("individual taxa boxplots.pdf", height=8, width=8) #height and width in inches
ind_boxplots
dev.off()
